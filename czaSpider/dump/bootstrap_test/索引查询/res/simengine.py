...
class SimEngine:
    ...
    def search(self, user_input, path='invertedIndex.json'):
        self._init_invertedIndex(path)
        self._get_result(user_input)
        return self
    ...
    def _get_result(self, user_input):
        print(user_input)
        print(reObj.sub('', user_input))
        for key in set(jieba.lcut(reObj.sub('', user_input))):
            print(key)
            if key in self._invertedIndex:
                self._result.extend(self._invertedIndex[key])


if __name__ == '__main__':
    # SimEngine().init()
    print(SimEngine().search('123').result)
