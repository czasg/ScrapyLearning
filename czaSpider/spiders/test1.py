...
def get_data(name, PartyFunctionCode):
    data = {
        ...
        "biz_content": {"vesselEnglishName": name,
                        "VesselCallNumber": "",
                        "IMONumber": "",
                        "VoyageNumber": "",
                        "PortCode": "",
                        ...
                        "PartyFunctionCode": PartyFunctionCode,  # todo
                        "CarrierName": ""},
    }
...
class MySpider(IOCO):
    ...
    url = "http://track.logink.org/vesselJSON.jsp"

    def parse(self, response):
        detail_url = "http://track.logink.org:9082/OceanTracking/VesselTracking"
        ret = []
        aims = data_from_xpath(response, '//div[@class="detail_table"]//tr[position()>1]/td[2]/text()', returnList=True)
        print(aims)
        # for aim in aims:
        #     for line in names.strip().split("\n"):
        #         try:
        #             line = line.strip()
        #             res1 = requests.get(detail_url, params=get_data(line, aim))
        ...
        # with open("船舶状态查询.json", "w", encoding="utf8") as f:
        #     f.write(json.dumps(ret, ensure_ascii=False))
        # with open("船舶状态查询-中文.json", "w", encoding="utf8") as f:
        #     f.write(json.dumps(list2china(ret), ensure_ascii=False))
