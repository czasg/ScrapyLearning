...
class MySpider(IOCO):
    ...
    def process(self, response):
        ...
        for house in houses:
            _item = item.copy()
            _item["house_name"] = data_from_xpath(house, './/h5[@class="title sign"]/a/text()', first=True)
            _item["house_area"], \
            ...
            url = data_from_xpath(house, './/h5[@class="title sign"]/a/@href', url=True, source=response)
            urls.append(url)
