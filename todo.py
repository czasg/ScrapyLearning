...
2020.03.29 - 简历需要重新修改一份，而且调度项目是否可以监控100台服务器，并获取对应的状态呢。这涉及到了一个运维的知识。
...
2020.04.28 - electron构建一个远程监控小玩具，可以使用python搭建对应的后端和截图功能
2020.04.28 - electron-vue这玩意，怎么感觉vue有点无敌的趋势，什么都可以写吗，可以参考参考
2020.04.28 - vue源码也是一条不归路啊
"""

"""DONE!
2020.04.24 - 验证码有两个bug，1、不能同时启动多个模型，不然会找不到路径? 2、保留率与图片准确率显然对不上，这是为什么
