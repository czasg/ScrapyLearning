...
在线聊天功能。像梦幻西游那种是怎么实现的啊。这个很重要!!!!
1、首页index，五个可伸缩的页面，第一页面直接指向下部分的数据展示
5、爬虫应该在最后，因为反爬还没有实现
2、blog博客页面，怎么排列，怎么展示，最重要的就是编写界面了。
而且博客应该是单独指向一个大页，而不是一个单纯的展示页面，很丑的。
博客很重要的一点就是如何编写，这里需要好好的添加其他的功能，因为要方便后序进行管理
3、tools工具的使用。
工具1：前端转字典的工具
工具2：前端颜色工具
4、简历的编写，这个还是需要写一个漂亮的历史线，这个不急。慢慢写。

# 最近的目标 #
socket聊天，如何在网页上实现，需要搭建对应的数据库。使用mongodb极好，但是一些其他的东西该如何实现呢，如片之类的应该也可以
"""